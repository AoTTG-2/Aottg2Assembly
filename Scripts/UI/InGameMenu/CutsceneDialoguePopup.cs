...
namespace UI
{
    class CutsceneDialoguePanel: BasePopup
    {
        ...
        protected override float Height => 160f;
        ...
        public override void Setup(BasePanel parent = null)
        {
            ...
            GameObject labelRight = ElementFactory.CreateDefaultLabel(group, style, "Press Y to skip", FontStyle.Normal, TextAnchor.MiddleRight);
            ElementFactory.CreateHorizontalGroup(group, 0f);
            ...
            _image = ElementFactory.CreateRawImage(group.transform, style, "Levi1Icon", 
                elementWidth: 64, elementHeight: 64).GetComponent<RawImage>();
            ...
            _contentLabel.GetComponent<LayoutElement>().preferredHeight = 64f;
            _contentLabel.GetComponent<LayoutElement>().preferredWidth = GetPanelWidth() - HorizontalPadding * 2f - 64f - 10f;
        }

        public void Show(string icon, string title, string content)
        {
            SetTitle(title);
            _image.texture = (Texture2D)AssetBundleManager.LoadAsset(icon + "Icon", true);
