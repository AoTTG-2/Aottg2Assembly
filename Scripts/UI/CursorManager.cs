    ...
    class CursorManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            else
            {
                ...
                if (menu.InMenu())
                    SetPointer();
                else if (!manager.IsFinishedLoading())
                    SetPointer();
                else if (manager.CurrentCharacter != null && manager.CurrentCharacter is Human)
                    SetCrosshair();
                else
                    SetHidden();
            }
            ...
            Text crosshairLabelRed, bool force = false)
        {
            if (_instance._ready)
            {
                if (State != CursorState.Crosshair || false) //InGameMenu.HideCrosshair)
                {
