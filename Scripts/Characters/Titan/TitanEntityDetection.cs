...
namespace Characters
{
    ...
    {
        protected HashSet<GameObject> _entities = new HashSet<GameObject>();
        protected HashSet<Hook> _hooks = new HashSet<Hook>();
        ...
        public static TitanEntityDetection Create(BaseTitan owner)
        {
            ...
            if (move is CapsuleCollider)
            {
                ...
                entityCollider.radius = capsuleMove.height * 2f;
        ...
        {
            Util.RemoveNull(_entities);
            Util.RemoveNull(_hooks);
            if (_entities.Count == 0 && _hooks.Count == 0)
                Detect = false;
        }
    }
