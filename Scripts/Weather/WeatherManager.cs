...
namespace Weather
{
    class WeatherManager : MonoBehaviour
    {
        ...
        {
            // if (SceneLoader.SceneName == SceneName.MainMenu && SettingsManager.GraphicsSettings.AnimatedIntro.Value)
            if (false)
            {
            ...
            else if (SceneLoader.SceneName == SceneName.CharacterEditor)
                _currentWeather.AmbientLight.Value = new Color(1f, 1f, 1f, 1f);
        }
                ...
                switch (effect)
                {
                    case WeatherEffect.Daylight:
                        foreach (Light light in MapLoader.Daylight)
                            light.color = _currentWeather.Daylight.Value;
                        break;
                    case WeatherEffect.AmbientLight:
