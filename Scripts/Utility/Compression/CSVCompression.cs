...
namespace Utility
{
    class CSVCompression
    {
        ...
        public static object[] Compress(string source, char containerDelimiter = ';', char rowDelimiter = ',')
        {
            ...
            foreach (string[] strArray in outputArr)
                outputList.Add(string.Join(",", strArray));
            byte[] compressed = StringCompression.Compress(string.Join("\n", outputList.ToArray()));
        ...
        public static string Decompress(byte[] source, JSONNode symbolTable, char containerDelimiter = ';', char rowDelimiter = ',')
        {
